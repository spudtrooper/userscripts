// ==UserScript==
// @name          Craigslist image preview 1.1
...
const KEEP_ORIGINAL_IMAGE_PARAM = "original.image";
...
function addLinkAtTop() {
  ...
  function setChecked(check, value) {
    try {setChecked_(check,value);} catch (e) {alert(e);}
  }
  function setChecked_(check, value) {
    if (value) {
  ...
  $t(" keep original image: ",span);
  check = $n("input",span);
  check.type = "checkbox";
  setChecked(check, keepOriginalImage);
  check.addEventListener("change",(function() {
    var check_ = check;
    return function() {
      var v = !!check_.checked;
      keepOriginalImage = setValue(KEEP_ORIGINAL_IMAGE_PARAM, String(v));
      // The user can reload
  ...
  check = $n("input",span);
  check.type = "checkbox";
  setChecked(check, keepAspectRatio);
  check.addEventListener("change",(function() {
    var check_ = check;
    return function() {
      var v = !!check.checked;
      keepAspectRatio = setValue(KEEP_ASPECT_RATIO_PARAM, String(v));
      changeSizes();
      };
    })(), true);
...
function main() {
  ...
  keepAspectRatio = getBoolean(KEEP_ASPECT_RATIO_PARAM, true);
  keepOriginalImage = getBoolean(KEEP_ORIGINAL_IMAGE_PARAM, false);
